...
class Agent:
    ...
    def move_event(self, event):
        ...
        if event.event_time <= self.sim.max_t:
            r_delta = random.choice([-1, +1])
            self.pos_y = self.pos_y + r_delta
            self.history_y.append(self.pos_y)
            self.pos_x = self.cst
            self.history_x.append(self.cst)
            # next event time. Random choice plus cst
            net = random.choice([1, 2]) + self.cst
            e = Event(net, self, 1)
    ...
    def initial_event(self):
        ...
        ime = Event(1, self, 1)
        self.add_event_to_simulation(ime)
...
def plot_random_walk(agents_x_hist, agents_y_hist):
...
def main():
    ...
    s.run()
    print(f'Agent: {a.uid} X: {a.history_x} Y: {a.history_y}\n')
    plot_random_walk(a.history_x, a.history_y)
